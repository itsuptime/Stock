...
client.on("ready", () => {
 ...
 client.on('guildCreate', guild => {
  ...
  let textList = [`${prefix}Stock`, `${prefix}help`, "By Aw Elfanaan"]
  client.user.setPresence({ activities: [{ name: `${prefix}help` }], status: 'dnd' })

  setInterval(() => {
    let text = textList[Math.floor(Math.random() * textList.length)]
    client.user.setPresence({ activities: [{ name: text }], status: 'dnd' })
  }, 5000)
})
